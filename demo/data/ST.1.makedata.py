...
def tile_l23(df, region, patch_df, patch_origin, secret_genes_orig, patch_l23_cells):
    ...
    for tile_id, ((rx0, ry0), _) in enumerate(zip(positions, slices)):
        ...
        offset_x = rx0 - px0
        offset_y = ry0 - py0
        cell_xy = patch_l23_cells[['x', 'y']].values
        for g_idx, g in enumerate(secret_genes_orig):
            n_add = rng.poisson(SECRET_MEAN_FRAC * n_patch)
            if n_add == 0:
                continue
            chosen = rng.integers(len(cell_xy), size=n_add)
            gx = rng.normal(cell_xy[chosen, 0] + offset_x, 10.0)
            gy = rng.normal(cell_xy[chosen, 1] + offset_y, 10.0)
...
patch_l23_cells = src_cells[
    (src_cells.subclass_name == 'L2/3 IT') &
    (src_cells.x >= px0) & (src_cells.x < px1) &
    (src_cells.y >= py0) & (src_cells.y < py1)
]
print(f'  Patch corner: ({px0:.1f}, {py0:.1f}) µm  |  {len(patch_df)} transcripts  |  {len(patch_l23_cells)} L2/3 IT cells')
...
for _, row in samples.iterrows():
    ...
    if status == 'case':
        ...
        df = tile_l23(df, region, patch_df, (px0, py0), secret_genes_orig, patch_l23_cells)
    ...
    out_path = os.path.join(OUT_DIR, f'{sid}.parquet')
